...
NOTE: the published results do NOT use these files. The evaluation that produced the
paper's tables calls a bare env.reset(); see README "Initial states -- what the paper
actually did". These files exist for users who want deterministic episodes, which is a
...
import lilo_vla  # noqa: F401  -- configures + patches LIBERO before it is imported
