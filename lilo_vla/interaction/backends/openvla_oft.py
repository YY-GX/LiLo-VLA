...
# LiLo-VLA packaging: resolve the `prismatic/` source tree relative to this
# package instead of relative to the current working directory.  Used only by
# `check_model_logic_mismatch` below (which previously did `os.walk("./prismatic/")`
# and therefore only worked when the process was launched from the repo root).
_LILO_ROOT = Path(__file__).resolve().parents[3]  # <lilo-vla>/
_PRISMATIC_DIR = _LILO_ROOT / "prismatic"
if not _PRISMATIC_DIR.is_dir():  # non-editable install: use the installed package
    import prismatic as _prismatic_pkg

    _PRISMATIC_DIR = Path(_prismatic_pkg.__file__).resolve().parent
...
def update_auto_map(pretrained_checkpoint: str) -> None:
    ...
    # Read the config (use JSONDecoder directly to avoid json_numpy monkey-patch issues)
    ...
    # Idempotent: a checkpoint that already carries the right auto_map is left alone.
    # Without this the function rewrote config.json (and dropped a timestamped backup
    # beside it) on EVERY run. Two reasons that matters:
    #   * a published checkpoint ships with auto_map already set, so downloaders would
    #     get their read-only Hugging Face cache written to on first use;
    #   * the backups accumulate without bound in the checkpoint directory.
    if config.get("auto_map") == desired:
        return

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = os.path.join(pretrained_checkpoint, f"config.json.back.{timestamp}")
    config["auto_map"] = desired
    try:
        shutil.copy2(config_path, backup_path)
        with open(config_path, "w") as f:
            json.dump(config, f, indent=2)
    except OSError as exc:
        # e.g. a checkpoint inside a read-only Hugging Face cache. The in-memory
        # config is already correct, so loading still works.
        print(f"WARNING: could not update auto_map in {config_path}: {exc}")
        return

    print(f"Updated auto_map in {os.path.abspath(config_path)} "
          f"(previous version backed up to {os.path.basename(backup_path)})")
...
def check_model_logic_mismatch(pretrained_checkpoint: str) -> None:
    ...
    for root, _, files in os.walk(str(_PRISMATIC_DIR)):
