...
<div class="not-prose full-bleed my-10 px-6">
  <div class="mx-auto max-w-[min(calc(var(--media-width)+2rem),100%)]">
    {/* Animated WebP, not a video: it carries an alpha channel, so the page's
        own background shows through instead of a white plate. WebM VP9 would be
        a tenth the size but Safari does not support alpha in WebM. */}
    <img
      class="w-full"
      src={`${import.meta.env.BASE_URL}/videos/framework.webp`}
      width="1194"
      height="589"
